...
import { createBinder } from "@/services/binder";
...
export function CreateBinderDialog({
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      await createBinder({ name: values.name, cupboard_id: cupboardId });
      ...
      form.setError("root", {
        type: "manual",
        message: "Erreur lors de la création du classeur",
      });
      console.error(err);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Créer un nouveau classeur</DialogTitle>
          <DialogDescription>
            Entrez le nom du nouveau classeur documentaire.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
